...
const BUSINESS_WHATSAPP = "263773835075"; // Main WhatsApp Business line for customer updates.
const BACKUP_WHATSAPP = "263775123428"; // Backup and admin support line.
...
function AdminPage() {
	...
	return <main className="admin-page"><section className="admin-header"><div><p className="eyebrow">Shared database dashboard</p><h1>LELE Runner orders</h1><p className="muted">Review deposits, update order progress and adjust the rates used on new customer quotes.</p></div><button className="admin-link" onClick={() => { request("/api/auth/logout", { method: "POST" }); setMode("login"); }}>Sign out</button></section><section className="admin-alert"><span>◉</span><div><strong>Order update centre</strong><p>Automatic WhatsApp updates will be enabled after Meta approves the LELE Runner WhatsApp Business review. For now, use the customer number shown on each order for manual updates.</p></div></section><section className="stats"><article><span>Awaiting deposit</span><b>{orders.filter((order) => order.status === "Awaiting deposit verification").length}</b><small>Needs payment review</small></article><article><span>In progress</span><b>{orders.filter((order) => !["Awaiting deposit verification", "Delivered"].includes(order.status)).length}</b><small>Placed to delivery</small></article><article><span>Orders total</span><b>{money(orders.reduce((sum, order) => sum + Number(order.total_amount), 0))}</b><small>All submitted orders</small></article></section><section className="admin-grid"><form className="card pricing-card" onSubmit={savePricing}><h2>Pricing settings</h2><p className="muted">Only new orders use updated rates. Existing quotes stay unchanged.</p>{([['markupRate','Service markup'], ['customsRate','Customs & logistics'], ['deliveryRate','Delivery'], ['depositRate','Deposit']] as Array<[keyof Pricing, string]>).map(([key, label]) => <label key={key}><span>{label} (%)</span><input type="number" min="0" step="0.1" value={pricing[key]} onChange={(event) => setPricing({ ...pricing, [key]: Number(event.target.value) })} /></label>)}<button className="button">Save percentages</button>{message && <p className="success">{message}</p>}</form><section className="card orders-card"><h2>All orders</h2>{error && <p className="error">{error}</p>}{orders.length === 0 ? <p className="muted">No customer orders yet.</p> : orders.map((order) => <article className="order-row" key={order.id}><div className="order-top"><div><div className="order-id-row"><strong>{order.order_code}</strong><StatusPill status={order.status} /></div><p>{order.customer_name} · {order.customer_phone}</p></div><select aria-label={`Change status for ${order.order_code}`} value={order.status} onChange={(event) => updateStatus(order.id, event.target.value)}>{STATUSES.map((status) => <option key={status}>{status}</option>)}</select></div><div className="order-meta"><span>{order.items.length} product{order.items.length === 1 ? "" : "s"}</span><span>Total {money(order.total_amount)}</span><span>Deposit {money(order.deposit_amount)}</span></div><ul>{order.items.map((item, index) => <li key={index}>{item.product_name || item.product_link || "SHEIN product"} · {money(item.unit_price)} × {item.quantity}</li>)}</ul><small>Payment reference: {order.payment_reference} · {order.delivery_method === "delivery" ? "Delivery" : "Collection"}</small></article>)}</section></section></main>;
}
